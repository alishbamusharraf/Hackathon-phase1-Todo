# Data Model: Todo CLI App Enhancement (Intelligent Features)
...
This document defines the data model for the enhanced Todo CLI application, which extends the basic todo functionality with recurring tasks, full datetime support, and reminder capabilities.

## Entity Definitions

### Task

The Task entity represents a single todo item with enhanced attributes for intelligent features.
...
class Task:
    ...
    due_datetime: Optional[datetime] = None  # Due date and time, None if not set
    recurrence: Optional[str] = None  # Recurrence pattern, e.g. "daily", "weekly", "every 3 days", None if not recurring
    next_occurrence: Optional[datetime] = None  # Next occurrence datetime for calculation, None if not recurring
...
- **due_datetime**: Due date and time for the task (datetime object, optional)
- **recurrence**: Recurrence pattern of the task (string, optional)
- **next_occurrence**: Next occurrence datetime for calculation (datetime object, optional)
...
- `recurrence` must be one of the supported patterns ("daily", "weekly", "monthly", "yearly", "every X days", etc.) or None
...
- When a recurring task is marked complete, a new task is created with the next occurrence date

### TaskList

The TaskList entity represents a collection of Task objects with enhanced functionality for intelligent features.
...
class TaskList:
    ...
    def add_task(self, title: str, description: str = "", priority: str = "medium",
                 tags: List[str] = None, due_datetime: Optional[datetime] = None,
    ...
    def get_overdue_tasks(self) -> List[Task]:
        """Get tasks that are overdue (due date is in the past and status is pending)"""
        pass
...
- `get_overdue_tasks`: Returns tasks that are overdue
- `get_upcoming_tasks`: Returns tasks that are due within a specified timeframe
- `handle_recurring_task_completion`: Handles the logic when a recurring task is completed
...
1. **Task Creation**: User provides title, description, priority, tags, due_datetime, and recurrence → Task object created with unique ID
2. **Task Storage**: Task objects stored in TaskList's internal list
3. **Task Retrieval**: TaskList provides methods to search, filter, sort, and get special categories (overdue, upcoming)
...
6. **Recurring Task Completion**: User marks recurring task as complete → New task created with next occurrence date
...
- Recurrence patterns must be valid as defined in the validation rules